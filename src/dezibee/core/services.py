...
def build_design_prompt(req: Requirement) -> str:
    ...
    parts.append(
        "工作顺序：\n"
        "1. 先读 demo/GUIDE.md；保留/创建第一页「需求概述」画布页，填写需求名称、版本信息、时间等概要信息。\n"
        "2. 从第二页画布开始拆解具体业务页面和状态，再编辑 WORKBENCH_DATA：建页面 → 建卡片 → "
        "建关系 → 写 PRD 章节 → 建立双向关联。\n"
        "3. PRD 第一章写本次需求背景和目标、需求概述；涉及多个页面流转时补充全流程文字说明。除第一页外，每个页面按以下顺序组织：业务需求概述、页面结构、元素说明（按区域写展示组件/字段/功能按钮）、业务规则（含交互/跳转/数据处理）、数据/API、报错及异常；不适用项可省略，其他开发所需信息按需补充。\n"
        "4. PRD 章节必须自动连续编号：一级 1.、二级 1.1.、三级 1.1.1.、四级 1.1.1.1.；新增内容沿用当前文档顺序继续编号，不重置、不跳号。页面级章节建议为二级编号，页面内小节为三级编号。\n"
        "5. 如需更完整的独立 PRD 文档，写到 prd/<主题>.md（可选）。\n"
        "6. 全部完成后再回复用户，概述本轮完成内容（页面/卡片/PRD 更新点），"
        "并提示可点「预览」查看；需要部署时直接复制 demo/ 文件夹。\n"
        "7. 后续用户反馈：先改原型，再同步更新对应 PRD 章节与关联。\n"
        "注意：这是敏捷迭代式 PRD，只覆盖本次实际调整；不要为了格式完整补写无关内容。"
    )
