# BeeHive 中 WokBee 运行模型过程逻辑（产品视角）

本文按当前代码行为描述 BeeHive 中 WokBee 项目功能的运行过程，重点回答三个问题：任务从哪里进入、Agent 如何决定下一步、结果如何保存和反馈。文末列出需要产品继续确认的风险与取舍。

## 一、产品对象

WokBee 是 BeeHive 中负责项目执行的功能模块，围绕一个“项目”运行。项目包含名称、目标、审核策略、项目目录、运行记录、交付物、脚本和项目经验。
...
BeeHive 中的 WokBee 现在是“项目驱动的 Agent 执行器”：项目目标负责提出任务，有序管线负责复用稳定流程，Agent 负责处理需要理解和判断的阶段，项目经验负责沉淀可复用方法。手动运行、AutoBee 和网关共享执行内核，但审批、是否走管线、是否允许等待用户这三个策略不同，是当前最需要在产品层面解释清楚的部分。
